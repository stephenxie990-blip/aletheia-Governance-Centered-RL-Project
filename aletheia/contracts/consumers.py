...
def update_hold_state_channels(
    ...
    active_channel_name = active_hold_channel(late_gate_mean)
    updated: dict[str, float] = {}
    for channel in HOLD_CHANNELS:
        if channel == active_channel_name:
    ...
    if updated["active_transition"] < updated["pre_transition"] * 0.5:
        updated["active_transition"] = max(updated["active_transition"], updated["pre_transition"] * 0.5)
    if updated["post_transition"] < updated["active_transition"] * 0.5:
        updated["post_transition"] = max(updated["post_transition"], updated["active_transition"] * 0.5)
    return updated
